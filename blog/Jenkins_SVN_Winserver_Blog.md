 ...
 * 下载上面的安装包以后，我们就开始安装SVNServer了，一路下一步就ok了，这里我指定端口为**4437**。
   ...
   ![](./img/installSVNServer1.png)

   ![](./img/installSVNServer2.png)
 ...
 * 然后找个文件夹CheckOut下来。因为我们是搭的本地SVN服务，路径就是https://localhost:4437/svn/ **{仓库名}**，这里 `{仓库名}`就是上面我们建的`Net5Repo`，
   ...
   ![](./img/checkoutRepo2.png)

 * 点击OK后会验证账户密码，输入正确以后就能把仓库拉取下来了。

   ![](./img/checkoutRepo.png)

**3. 新建Net5.0项目,并传至svn仓库**
 * 打开VS2019新建Net5.0项目，

   ![](./img/createVSProJ1.png)

 * 这里我们就建一个名为NetApi的Web项目

   ![](./img/createVSProJ2.png)

 * 注意选择NetCore版本

   ![](./img/createVSProJ3.png)
 ...
 * 因为`net 5.0`集成了`swagger`，因此我们可以直接访问Api地址: https://localhost:5001/swagger/index.html
   ...
   ![](./img/runProj2.png)
 * swagger可以代替接口文档对接前端，实际开发中只要把这个地址丢过去就行了，简直不要太完美。![](./img/)
 ...
 * 首先安装Jenkins
      * 这里把端口指定为**8888**。
        ...
        ![](./img/installJenkins1.jpg)
      * 然后按提示步骤一步步下去。

        ![](./img/installJenkins2.png)
      * 这里要注意选择**安装推荐的插件**，可以省不少事。

        ![](./img/installJenkins3.png)
      * 安装完插件重启后，会让你创建管理员，这里我们创建一个`admin`账户。

        ![](./img/installJenkins4.png)
 * 然后登录jenkins

   ![](./img/loginJenkins.png)

 * 安装`Subversion`插件，依次点击`Manage Jenkins`->`Manage Plugins`,搜索`Subversion`勾选安装并重启。
 * 接下来就开始配置任务了，我们在首页点击`新建Item`，任务名就叫`NetApi`，选择`FreeStyle Project` (yeah~ freestyle!)![](./img/bq2.gif)
 ...
 2. `构建触发器`里定义条件，选择`Poll SCM`(具体规则可以点击右侧的![](./img/wenhao.png)查看)，这里我们设定5分钟检测一次 `H/5 * * * *`
   ![](./img/createItem4.png)
 ...
 4. 这里我们以Jenkins构建后的工作区（workspace）为主，编译发布`NetCore`项目，然后建立成服务。
      ...
      * 然后打开浏览器访问 http://localhost:8800/swagger/index.html 可以看到服务起来了。
        ...
        ![](./img/createItem10.png)

      * 打开任务管理器也能看到

        ![](./img/createItem11.png)
 ...
 * 直接打开swagger页面，刷新下，自动发布更新了。。![](./img/bq4.jpg)
